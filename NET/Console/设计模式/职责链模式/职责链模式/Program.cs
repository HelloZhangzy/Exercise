...
namespace 职责链模式
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Dictionary<string, object> ls2 = new Dictionary<string, object>();
            ls.Add("A1", 1);
            ls.Add("A2", 2);
            ls2.Add("B1", 1);
            ls2.Add("B2", 2);
            ...
            string ret = new JavaScriptSerializer().Serialize(ls);
            JObject request = (JObject)JsonConvert.DeserializeObject(ret);
            JObject B = (JObject)request["A3"];
            int d= (int)request["A3"]["B2"];
            Console.WriteLine(B["B1"]);
            Console.WriteLine(d);
            Console.Write( Convert.ToBoolean(int.Parse("0")));
    ...
    class Director : Approver
    {
        ...
        public override void processRequest(PurchaseRequest request)
        {
            ...
            {
                Console.WriteLine("主任" + this.name + "审批采购单：" + request.getNumber() + "，金额：" + request.getAmount() + "元，采购目的：" + request.getPurpose() + "。");  //处理请求  
            }
            else
            {
